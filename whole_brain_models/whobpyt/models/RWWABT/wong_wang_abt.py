...
class RNNRWWABT(AbstractNMM):
    ...
    def __init__(self, node_size: int,
                 TRs_per_window: int, step_size: float, sampling_size: float, tr: float, sc: float, ab: float, tau: float, use_fit_gains: bool,
        ...
        # ABeta-Tau constants
        self.ab = ab  # A-beta tensor: node_size x 1 ? 
        self.tau = tau # tau tensor: node_size x 1 ?
...
def integration_forward(model, external, hx, hE, softplus_threshold = 20):
    ...
    # aB-tau model params
    bAB_E = model.params.bAB_E.value()
    sAB_E = model.params.sAB_E.value()
    bt_E = model.params.bt_E.value()
    st_E = model.params.st_E.value()
    bAB_I = model.params.bAB_I.value()
    sAB_I = model.params.sAB_I.value()
    bt_I = model.params.bt_I.value()
    st_I = model.params.st_I.value()
    ...
    if model.use_dynamic_boundary:
        for TR_i in range(model.TRs_per_window):
            ...
            for step_i in range(model.steps_per_TR):
                ...
                mE = calc_gain_factor(bAB_E, sAB_E, model.ab, bt_E, st_E, model.tau)
